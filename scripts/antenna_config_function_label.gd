...
func _ready():
	...
	self.label.position = Vector2(-length/2, -2*width/3)
	self.label2.size = Vector2(length, width)
	self.label2.position = Vector2(-length/2, -width/3.5)
	appear_with_anime()
...
func set_label2_text(text:String):
	label2.text = text
