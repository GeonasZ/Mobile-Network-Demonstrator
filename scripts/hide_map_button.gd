...
func button_click_function():
	path_layer.map_visible = ! path_layer.map_visible
	if path_layer.map_visible:
		self.char.text = "H"
		self.label.set_text("Hide Map")
		path_layer.redraw()
	else:
		self.char.text = "S"
		self.label.set_text("Show Map")
		path_layer.redraw()
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventKey and (event.keycode == KEY_H and path_layer.map_visible or event.keycode == KEY_S and not path_layer.map_visible) and event.is_pressed():
		on_work = false
